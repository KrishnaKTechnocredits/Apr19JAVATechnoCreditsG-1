...
public class MaxValueAtDifferIndex {
	void findMaxValueAtDifferIndex(int[] firstArray,int[] secondArray)
	{
		boolean flag=false;
		...
		else
		{
			for(int i=0, j=0;j<secondArray.length;j++,i++)
			{
				...
				{
					flag=true;
					System.out.print("Values are not matching at index -> "+i);
					if(firstArray[i]>secondArray[i])
						System.out.println(" and Max value is "+firstArray[i]);
					else
						System.out.println(" and Max value is "+secondArray[i]);
					//break;
				}
			}
			if(flag==false)
	...
	public static void main(String args[])
	{
		...
		maxValueAtDifferIndex.findMaxValueAtDifferIndex(firstArray,secondArray);
	}

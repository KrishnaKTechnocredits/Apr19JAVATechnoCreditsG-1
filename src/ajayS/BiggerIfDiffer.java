...
public class BiggerIfDiffer {
	void biggerValue(int[] array1, int[] array2) {// method to match all the
		...
		for (int i = 0; i < array1.length; i++) {
			if (array1[i] != array2[i]) {
				...
				if (array1[i] > array2[i])
					System.out.println("from (" + array1[i] + ", " + array2[i] + "max value is : " + array1[i]);
				else
					System.out.println("from (" + array1[i] + ", " + array2[i] + "max value is : " + array2[i]);
			}
		}
		if (count == 0)
	...
	public static void main(String[] args) {
		...
		BiggerIfDiffer biggerIfDiffer = new BiggerIfDiffer();
		biggerIfDiffer.biggerValue(array1, array2);
		scanner.close();
	}
